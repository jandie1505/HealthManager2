        ...
        if(sender instanceof Player){
            ...
            if(!Main.ignoreop && p.isOp() || p.hasPermission("healthmanager.getsaturation")){
                if(args.length == 1){
                    ...
                    try {
                        if(target != null){
                            SendMessage.defaultMessage(p,Messages.getGetsaturationMessage(target.getName(), (int) target.getSaturation()));
                        } else {
            ...
            if(args.length == 1){
                ...
                try {
                    if(target != null){
                        ConsoleMessages.noPrefixMessage(Messages.getGetsaturationMessage(target.getName(), (int) target.getSaturation()));
                    } else {
